<?php
require_once('class.fireSQL.php');
...
class Payment extends fireSQL{
		...
		function find_by_customer_id($customerID){
			$customerData = $this->select('users', array('userID'), array('customerID'=>$customerID));
			...
			while($row = $paymentProfilesObj->fetch_assoc()){
				...
				$resultHtml .= '<tr >
                   <td class="table-checkbox">
                    ...
                    <td>'.$card["cardType"].'</td>
                    <td></td>
                    <td><button type="button" class="btn fs-btn-orange btn-sm pull-right updateSubscriptionPayment" data-id="'.$card["paymentID"].'">Set for Subscription</button></td>
                ...
                $resultHtml .= '</td>
                    <td><button type="button" class="btn fs-btn-orange btn-sm pull-right delete-payment" data-id="'.$card["paymentID"].'">Delete</button></td>   
                  </tr>';
			}
			return $resultHtml;
		}
