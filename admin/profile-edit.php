      ...
      <nav class="navbar">
          <div class="navbar-collapse">
            <ul class="nav nav-justified">
                ...
                <li class="active"><a href="profile-edit.php?user=<?php print($userID); ?>" title="">Edit Profile</a></li>
   ...
   <div class="container info">
    ...
    <div class="container info">
            ...
            <div class="row bg-grey">
              <div class="col-md-10">
                  <h4 class="fs-header text-left">Chapters <span class="fs-strong">Enrolled</span></h4>
                   <ul class="list-inline groups enrolledChapters">
                        <?php print($chaptersEnrolled); ?>
                    </ul>
                <div class="btn-group chapter">
                    <button type="button" class="btn btn-default fs-btn-orange btn-sm pull-right" id="addChapterBtn">Assign Chapter</button>
                </div>
              </div>
            </div>
        </div>
      ...
      <div class="row" id="addChapter">
        <div class="col-md-10 col-md-offset-1">
            <form class="form-horizontal fs-form-gen">
                <div class="form-group text-left">
                  <div id="header">
                    <h3 class="fs-header">Assign <strong>Chapter</strong></h3>
                  </div>
                </div>
                <div class="form-group">
                      <select type="text" class="form-control" id="chapters" name="chapters">
                            <option value="">- select -</option>
                            <?php print($chapters->printChaptersOption(''));?>
                        </select>
                   </select>
                </div>
            </form>
                ...
                <form class="form-horizontal fs-form-gen" method="POST" name="profileContactInfo" id="profileContactInfo">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-12 col-sm-12 col-md-3">
                            ...
                            <input type="text" class="form-control" id="email" name="email" placeholder="E-mail" value="<?php print($profInfo["email"]);?>">
                        </div>
                        <div class="col-xs-12 col-sm-12 col-md-3">
    ...
    function addRemoveListener() {
        $('.removeEnrolledChapter').on('click',function(){
         sendObj = {
            'userID': <?php print($userID);?>,
            'chapterID': parseInt($(this).attr('data-id'))
         }
         $.post("removeUserFromChapter.php", sendObj, function(result){
                if(result != "Error") {
                    $('.enrolledChapters').empty().append(result);
    ...
    $(document).ready(function(){
          ...
          userID = '<?php print($userID); ?>';
          companyIndustry = '<?php print($profInfo["companyIndustry"]);?>';
          
          if(companyIndustry != "") {
            $('#companyIndustry').val(companyIndustry);
          }
          ...
          $('textarea, input').on('blur',function(e) {
                ...
                sendObj = {'fieldName': fieldname, 'fieldVal': fieldval, 'type': 'text', 'userID': userID};
                
                if($(this).attr('type') != 'file' && fieldname != 'item'){

                    $.post("saveUserInfoAdmin.php", sendObj, function(result) {
                    });
