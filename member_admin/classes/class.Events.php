<?php 
	class Events extends fireSQL{
		...
		function getEvents($userID = null){
			$resultHTML = '';
			...
			if(!$userID){
				...
				$data = $this->select('eventsEnrolled', array('eventID'), array('userID' => $userID));
					print_r($data);
					if($data){
						$resultHTML = '';
						while($row = $data->fetch_assoc()){
							$event = $this->select('events', array('*'), array('eventID' => $row["eventID"]));
