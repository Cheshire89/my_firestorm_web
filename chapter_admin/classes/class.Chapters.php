<?php 
	class Chapters extends fireSQL{
	    ...
	    function getListChapters(){
	    	$resultArr = array();
	    	$data = $this->getAdminChaptersData();
		...
		function getAdminChaptersData(){
			$userID = $_SESSION["userID"];
			$db = db::instance();
		    $chaptersIDs = array();
		    
		    $adminData = $this->select('chapterAdmins', array('chID'), array('userID'=>$userID));
		    
			    while($row = $adminData->fetch_assoc()){
			    	array_push($chaptersIDs, $row);
			    }
		    
		    $i=1;
		    $chLen = count($chaptersIDs);
		    $query = "SELECT * FROM chapters WHERE ";

		    foreach($chaptersIDs as $chId){
		    	$chID = $chId["chID"];
		    	if($i != $chLen){ $query .= "chapterID = $chID || "; }else{ $query .= "chapterID = $chID;";}
		    	$i++;
		    }

		  	$chaptersData = $db->query($query);

			if($chaptersData->num_rows != 0){
				return $chaptersData;
			}else{
